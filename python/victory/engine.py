...
try:
    ...
    logger.error(f"Failed to import Victory modules: {e}")
    logger.error("Make sure victory package is installed: pip install -e python/")
...
async def handle_plan_command(command: Dict[str, Any]) -> None:
    ...
    try:
        ...
        # Create LLM provider
        try:
            provider = LLMProviderFactory.create(provider_name, api_key, model or "default")
        ...
        try:
            ...
            plan_text = response_obj.content if hasattr(response_obj, 'content') else str(response_obj)
        ...
        send_response(Response(
            status="error",
            action="stop",
            message=f"Plan command failed: {str(e)}"
        ))
...
async def handle_issue_command(command: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if not github_token:
            from victory.core.github import print_token_guide, get_github_token
        ...
        try:
            ...
            response_obj = await provider.generate(messages)
            patch = response_obj.content if hasattr(response_obj, 'content') else str(response_obj)
        ...
        send_response(Response(
            status="error",
            action="stop",
            message=f"Issue command failed: {str(e)}"
        ))
...
async def handle_fix_command(command: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if not github_token:
            from victory.core.github import print_token_guide
        ...
        try:
            ...
            if github_login:
                ...
                if github_login == authenticated_login or github_login == "me":
                    logger.info(f"Fetching issues for authenticated user: {authenticated_login}")
                    issues = await github.fetch_user_issues(authenticated_login, state="open")
                else:
                ...
                if len(parts) >= 2:
                    ...
                    issues = await github.fetch_repo_issues(owner, repo_name, state="open")
                else:
...
async def handle_create_pr_command(command: Dict[str, Any]) -> None:
    ...
    try:
        ...
        branch_name = f"victory-fix-{issue_number}"
        ...
        try:
            ...
            # 3. Commit
            run_git(["add", "."])
            run_git(["commit", "-m", f"Fix issue #{issue_number} using Victory AI"])
            ...
            pr_body = f"This PR was automatically generated by Victory AI to fix issue #{issue_number}.\n\n"
...
async def handle_apply_command(command: Dict[str, Any]) -> None:
    ...
    try:
        ...
        send_response(Response(
            status="error",
            action="stop",
            message=f"Apply command failed: {str(e)}"
        ))
...
async def handle_logs_command(command: Dict[str, Any]) -> None:
    ...
    try:
        ...
        log_dir = Path.home() / ".victory"
        log_file = log_dir / "victory.log"
        ...
        send_response(Response(
            status="error",
            action="stop",
            message=f"Logs command failed: {str(e)}"
        ))
...
async def handle_token_command(command: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if subcommand == "info":
            ...
            from victory.core.token_manager import init_token_manager, TokenBudget
        ...
        send_response(Response(
            status="error",
            action="stop",
            message=f"Token command failed: {str(e)}"
        ))
...
VICTORY_CONFIG_DIR = Path.home() / ".victory"
VICTORY_CONFIG_FILE = VICTORY_CONFIG_DIR / "config.json"


def load_config() -> Dict[str, Any]:
    """Load Victory configuration from ~/.victory/config.json"""
    if VICTORY_CONFIG_FILE.exists():
        try:
            with open(VICTORY_CONFIG_FILE, 'r') as f:
                return json.load(f)
        except Exception as e:
...
def save_config(config: Dict[str, Any]) -> None:
    """Save Victory configuration to ~/.victory/config.json"""
    try:
        VICTORY_CONFIG_DIR.mkdir(parents=True, exist_ok=True)
        with open(VICTORY_CONFIG_FILE, 'w') as f:
            json.dump(config, f, indent=2)
        os.chmod(VICTORY_CONFIG_FILE, 0o600)
    except Exception as e:
...
async def handle_config_command(command: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if action == "init":
            ...
            print("\n" + "="*60)
            print("🚀 Victory Setup - First Time Configuration")
            print("="*60 + "\n")
            ...
            provider = input("Select LLM provider (modal/openai/gemini/claude): ").strip().lower()
            ...
            if setup_github == 'y':
                ...
                print("Skipped. You can add it later with: victory config --github YOUR_TOKEN\n")
            ...
            framework = input("Enter test framework (or press Enter for auto-detect): ").strip()
            if framework:
            ...
            print("="*60)
            print("\nNext steps:")
            print("  - victory issue 42  # Fix a GitHub issue")
            print("  - victory plan 'Add error handling'  # Plan a fix")
            print("  - victory logs  # View activity")
            print("\nTo update config later: victory config")
            print("="*60 + "\n")
            
            send_response(Response(
                status="success",
                action="stop",
                message="Victory initialized successfully!",
                data={"config": config}
            ))
            return
        ...
        if action == "github":
            ...
            if token:
                ...
                send_response(Response(
                    status="success",
                    action="stop",
                    message="GitHub token saved! You can now use victory issue without entering token."
                ))
            else:
                print_token_guide()
