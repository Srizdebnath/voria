        ...
        for iteration in range(1, self.MAX_ITERATIONS + 1):
            ...
            try:
                ...
                if action == LoopAction.SUCCESS:
                    ...
                    logger.info(f"Tests failed, analyzing for iteration {iteration + 1}...")
...
Please refine your approach and generate an improved fix."""
    ...
    async def _step_test(self, state: LoopState) -> LoopAction:
        ...
        try:
            ...
            logger.info(f"Tests complete: {state.test_results.passed}/{state.test_results.total} passed")
        ...
        try:
            if state.test_results:
                analysis = await self.provider.analyze_test_failure(
                    test_output=state.test_results.stdout,
                    code_context=state.patch
                )
                
                logger.info(f"Analysis complete: {analysis.get('suggestions', '')[:200]}")
...
{analysis.get('suggestions', 'Try different approach')}"""
    ...
    def _format_result(self, state: LoopState, status: str) -> Dict[str, Any]:
        ...
        return {
            ...
            } if state.test_results else None,
            "errors": state.errors,
...
async def demo_agent_loop():
    ...
    result = await loop.run(
        issue_id=123,
        issue_description="Fix Unicode parsing in parser.py"
    )
